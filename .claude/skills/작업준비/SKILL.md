...
"/mnt/c/Program Files/Google/Chrome/Application/chrome.exe" \
  "https://claude.ai" \
  "https://current-rho.vercel.app/" \
  "https://supabase.com/dashboard/project/zlpdtswbgteufzszjigy" \
  "http://localhost:3003" &>/dev/null &
```
...
if lsof -i :3003 &>/dev/null; then
  echo "Dev 서버가 이미 실행 중입니다 (port 3003)"
else
  cd ~/current && PORT=3003 npm run dev &
fi
```
...
`/home/chosangyun/current/TODO.md`를 읽고:
